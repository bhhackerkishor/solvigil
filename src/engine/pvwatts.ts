...
export function computeSolarPosition(
  ...
  const cosAz =
    ...
    (Math.cos(latRad) * Math.cos(altitude * DEG_TO_RAD) || 0.0001);
  let azimuthFromSouth = Math.acos(Math.max(-1, Math.min(1, cosAz))) * RAD_TO_DEG;
...
export function computeGTI(
  ...
  const kt = ghi > 0 ? Math.min(ghi / 1000, 1) : 0;
  const diffuseFraction = 0.16 + 0.84 * (1 - kt);

  const directComponent = dni * Math.max(0, Math.cos(aoiRad));
  const diffuseComponent = ghi * diffuseFraction * ((1 + Math.cos(tiltRad)) / 2);
...
export function calculateExpectedPVWatts(input: PVWattsInput): PVWattsOutput {
  ...
  const solarTime = getLocalSolarTime(date, input.longitude);
  ...
  const gtiWm2 = computeGTI(input.ghi, input.dni, aoiDeg, input.tiltAngle);
...
function getLocalSolarTime(date: Date, longitude: number): number {
